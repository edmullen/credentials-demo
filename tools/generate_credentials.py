...
def wallet_person(person: dict, employer_names: dict[str, str]) -> dict:
    ...
    `employerNames` is a demo aid for the person switcher (Loop 4a, docs/design.md §7). The
    Wallet's connect logic never reads it: whether someone works for an employer is Payroll's
    call.
    """
    return {
        "id": person["id"],
