...
3. **Sample data** (#6) — **done**, in [docs/sample-data.md](../sample-data.md), generated by
   `tools/generate_sample_data.py` from hand-written YAML. 25 fictional people, 15 fictional
   NJ employers, 64 September 2026 paystubs, and 23 AI-generated identity photos. Includes the
   expected eligibility outcome for every person — Loop 6's test oracle.
...
## Open questions
- What format does the sample data ship in? Settled: people and employers are hand-written
  YAML, and a one-shot generator under `tools/` derives the rest into committed JSON. Nothing is
  copied into an app until Loop 2 (Wallet) and Loop 3 (Payroll).
